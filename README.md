...
You can run the example scripts in "dry-run" mode to simulate an oscilloscope without needing physical hardware. This is useful for testing the library or just seeing how it works.

To run the main script in dry-run mode:
```
python oscope.py --dry-run --hardcopy outfile.png --waveform 1 wave.npz
```
This will:
- Simulate a connection to a fake Keysight MSO-X 3054A
- Generate a synthetic waveform for Channel 1
- Save a placeholder screen image to `outfile.png` (in current directory)
- Save the waveform data to `wave.npz` (in current directory)

To plot the data (or run the plotter in dry-run mode):
```
python oscopeplot.py wave.npz
# OR
